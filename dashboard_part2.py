...
def display_pollutant_levels_analysis(water_df_full, meteorological_df_full, volcanic_df_full):
    ...
    st.markdown("Provides detailed trends for key water pollutants: Ammonia, pH Level, Nitrate, and Phosphate, with options to overlay related environmental data.")

    # --- NO SIDEBAR FILTERS FOR THIS SECTION ---
    # The data will be displayed for all available locations and the full date range

    # Use the full dataframes directly as no filters are applied in this section
    pollutant_filtered_df_water = water_df_full.copy()
    met_filtered_df = meteorological_df_full.copy()
    volc_filtered_df = volcanic_df_full.copy()

    if pollutant_filtered_df_water.empty:
        st.warning("No water quality data available for pollutant analysis.")
        return
    
    st.sidebar.subheader("Display Options")
    display_option = st.sidebar.radio(
        "Choose data combination:",
        ['Water Quality Only', 'Water Quality + Meteorological', 'Water Quality + Volcanic Activity', 'Water Quality + Meteorological + Volcanic Activity'],
    ...
    if not available_pollutants:
        st.warning("No relevant pollutant data (Ammonia, pH, Nitrate, Phosphate) found in the water quality data.")
        return
    ...
    st.info("These charts display the trend of individual pollutant levels from water quality data over time, categorized by location.")
    ...
    if display_option == 'Water Quality + Meteorological' or display_option == 'Water Quality + Meteorological + Volcanic Activity':
        ...
        available_met_params = [p for p in meteorological_plot_params if p in met_filtered_df.columns] # Use met_filtered_df here

        if not met_filtered_df.empty and available_met_params: # Use met_filtered_df here
            for param in available_met_params:
                st.write(f'Trend of {param} Over Time by Location:')
                try:
                    line_chart_met_param = met_filtered_df.pivot_table( # Use met_filtered_df here
                    ...
                    st.dataframe(met_filtered_df[['Date', 'Location', param]]) # Use met_filtered_df here
            st.write("Below is the raw meteorological data for all available locations and the full date range:")
            st.dataframe(met_filtered_df, use_container_width=True) # Use met_filtered_df here
        else:
            st.info("Meteorological data not loaded or no relevant parameters available.")
        st.markdown("---")
    ...
    if display_option == 'Water Quality + Volcanic Activity' or display_option == 'Water Quality + Meteorological + Volcanic Activity':
        ...
        available_volc_params = [p for p in volcanic_plot_params if p in volc_filtered_df.columns] # Use volc_filtered_df here

        if not volcanic_df_full.empty and available_volc_params: # Use volc_filtered_df here
            for param in available_volc_params:
                st.write(f'Trend of {param} Over Time by Location:')
                try:
                    line_chart_volc_param = volc_filtered_df.pivot_table( # Use volc_filtered_df here
                        index='Date', columns='Location', values=param, aggfunc='mean'
                    ).fillna(0)
                    st.line_chart(line_chart_volc_param)
                except Exception as e:
                    st.warning(f"Could not render chart for {param}. Error: {e}")
                    st.dataframe(volc_filtered_df[['Date', 'Location', param]]) # Use volc_filtered_df here
            st.write("Below is the raw volcanic activity data for all available locations and the full date range:")
            st.dataframe(volc_filtered_df, use_container_width=True) # Use volc_filtered_df here
        else:
            st.info("Volcanic activity data not loaded or no relevant parameters available.")
        st.markdown("---")
